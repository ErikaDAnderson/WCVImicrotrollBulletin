...
kableExtra::kable(sumfish,
                  ...
                  booktabs = TRUE,
                  caption = "Summary of Salmon Caught and Samples Collected. Acronymns include AD = adipose fin, CN = chinook, CO = coho, and Unk = unknown clip status.") %>%
...
```{r FishingSetDetails, fig.align = 'center', fig.cap = "Spread of fishing days across statistical weeks and locations"}
...
Preliminary information on prey items identified in sampled juvenile stomachs are shown below.
```{r Stomach Content."}
...
Preliminary information on clinical signs observed in caught juvenile salmon are shown below. Clinical signs categorized as "other" in the field are further broken down based on the sampling crew's descriptions. 
...
After samples are transported to the Pacific Biological Station, lab analysis will occur: gill samples and other tissues processed for infectious agents,  stressors and diseases using FitChips; stomachs contents identified and quantified; genetic stock identification estimated using SNPs (single nucleotide polymorphisms); energy density measured; and marine residency estimated from otoliths. Results will be communicated upon analysis completion.
