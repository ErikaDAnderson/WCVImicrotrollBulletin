...
library(bcmaps) # quality bc basemaps
...
####### change this as new data is entered
excelFilename <- "WCVI_template_beta_v1-3.xlsx"

# load data
site_orig <- read_excel(here::here("Input", excelFilename),
                        sheet = "01 SITE")

# *** blank column at 21 needs name "blank" or to be omitted
sets_orig <- read_excel(here::here("Input", excelFilename),
                        sheet = "02 FISHING SETS")

ocean_orig <- read_excel(here::here("Input", excelFilename),
                        sheet = "03 OCEANOGRAPHIC DATA")

fish_orig <- read_excel(here::here("Input", excelFilename),
                        sheet = "04 FISH")

stomach_orig <- read_excel(here::here("Input", excelFilename),
                        sheet = "05 STOMACH CONTENTS")
...
clayoquotMap <- mapSoundfn("CLAYOQUOT", mapdf, "low")
quatsinoMap <- mapSoundfn("QUATSINO", mapdf, "low")
barkleyMap <- mapSoundfn("BARKLEY", mapdf, "low")

```

### Quatsino Sound
```{r quatsinoMap, fig.height = 2.5, fig.cap="Quatsino Sound Catch and Fishing Effort."}
quatsinoMap
```
  
### Clayoquot Sound
```{r clayMap, fig.height = 3, fig.cap="Clayoquot Sound Catch and Fishing Effort."}
clayoquotMap
```
  
### Barkley Sound
```{r barkleyMap, fig.height = 3, fig.cap="Barkley Sound Catch and Fishing Effort."}
barkleyMap
```
...
# select species
thisdf <- filter(lengthFreq, species == thisspecies)
...
### Quatsino Chinook Salmon Lengths and Weights  
```{r lwQuatsinoCK, fig.height = 3, fig.cap="Lengths and Weights of Quatsino Juvenile Chinook Salmon."}

# select this sound only
lengthFreqQuatsino <- lengthFreq %>%
  filter(sound == "QUATSINO")

# create graphs
QuatsinoCK <- graphSpecies(lengthFreqQuatsino, "CN")
QuatsinoCK

```

### Clayoquot Chinook Salmon Lengths and Weights  
```{r lwClayoquotCK, fig.height = 3, fig.cap="Lengths and Weights of Clayoquot Chinook Salmon."}

# select this sound only
lengthFreqClayoquot <- lengthFreq %>%
  filter(sound == "CLAYOQUOT")

# create graphs
ClayoquotCK <- graphSpecies(lengthFreqClayoquot, "CN")
ClayoquotCK

```

### Barkley Chinook Salmon Lengths and Weights  
```{r lwBarkleyCK, fig.height = 3, fig.cap="Lengths and Weights of Barkley Chinook Salmon."}

# select this sound only
lengthFreqBarkley <- lengthFreq %>%
  filter(sound == "BARKLEY")

# create graphs
BarkleyCK <- graphSpecies(lengthFreqBarkley, "CN")
BarkleyCK
